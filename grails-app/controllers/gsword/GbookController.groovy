...
if (!pray) pray=new XmlSlurper().parse(new File(grailsApplication.config.prayerroot.toString()+"/praybible_zh.xml"))
	println "in prayer vp:"+(grailsApplication.config.prayerroot.toString()+"/praybible_zh.xml")
	//println pray
def cs=pray.c
...
if(params.comment) sendMail {
  ...
  private URL scheduletxt= getClass().getResource( "/daily.txt");
  private URL threeyearscheduletxt = getClass().getResource( "/threeyeardaily.txt");
  private threeplan=new File( grailsApplication.config.keyroot +"/threeyeardaily.txt")
...
"Rev"] as List
  ...
  private final URL xslurl = getClass().getResource( "/iBD.xsl");
